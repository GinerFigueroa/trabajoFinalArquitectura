...
include_once('../../../../../modelo/EntidadAuxiliarDAO.php'); 
...
class EvolucionSeguimientoDTO {
    ...
    public function __construct(array $data) {
        $this->idSeguimiento = $data['idSeguimiento'] ?? null;
    ...
    public static function crearComando(string $action, EvolucionSeguimientoDTO $dto): Comando {
        switch ($action) {
            case 'editar':
                ...
                return new EditarSeguimientoCommand($dto);
            default:
...
class RequeridosValidator extends AbstractValidatorHandler {
    ...
    {
        if (empty($dto->idSeguimiento) || empty($dto->idInternado) || empty($dto->idMedicoUsuario) || empty($dto->evolucion)) {
            return "Los campos ID Seguimiento, Paciente, Médico y Evolución son obligatorios.";
        }
        return $this->passNext($dto);
    }
}

// Handler Concreto 2: Validación de Médico
...
// Handler Concreto 3: Validación de Enfermera (si aplica)
    ...
    public function handle(EvolucionSeguimientoDTO $dto): ?string
    {
        if (!empty($dto->idEnfermeraUsuario)) {
            ...
            if (!$this->objAuxiliarDAO->validarUsuarioEsEnfermera($dto->idEnfermeraUsuario)) {
                return "El ID de usuario seleccionado como enfermera no es válido.";
            }
        }
        return $this->passNext($dto);
    }
...
// COMMAND Concreto: Editar Seguimiento 📦
class EditarSeguimientoCommand implements Comando
{
    // Atributos: DTO y Receptors (DAOs)
    private $objSeguimientoDAO; // Receptor 1 (InternadoSeguimientoDAO)
    private $objAuxiliarDAO; // Receptor 2 (EntidadAuxiliarDAO)
    private $dto;
    ...
    public function execute(): bool
    {
        ...
        // Método: `editarSeguimiento`
        return $this->objSeguimientoDAO->editarSeguimiento(
            $this->dto->idSeguimiento,
...
class controlEditarPacienteHospitalizado
{
    ...
    public function ejecutarComando(string $action, array $data)
    {
        ...
        $rutaRetorno = "./indexEditarPacienteHospitazado.php?id={$data['idSeguimiento']}";
        ...
        try {
            ...
            if ($mensajeError) {
                ...
                $this->objMensaje->mensajeSistemaShow(
                    '❌ Error al actualizar la evolución en la base de datos.', 
                    $rutaRetorno, 
                    'error'
                );
            }
